...
program
    ...
    try {
        ...
        if (gpxData.description)
            console.log(`  📄 描述: ${gpxData.description}`);
        if (gpxData.time)
            console.log(`  📅 时间: ${gpxData.time.toLocaleString()}`);
    ...
    try {
        ...
        if (fitData.session) {
            ...
            if (session.avg_cadence)
                console.log(`    🦵 平均踏频: ${session.avg_cadence} rpm`);
            if (session.max_cadence)
                console.log(`    🦵 最大踏频: ${session.max_cadence} rpm`);
            if (session.avg_power)
                console.log(`    ⚡ 平均功率: ${session.avg_power} W`);
            if (session.max_power)
                console.log(`    ⚡ 最大功率: ${session.max_power} W`);
        ...
        if (fitData.records.length > 0) {
            ...
            for (let i = 0; i < Math.min(3, fitData.records.length); i++) {
                ...
                if (record) {
                    ...
                    if (record.altitude)
                        console.log(`       海拔: ${record.altitude.toFixed(1)}m`);
                    if (record.heart_rate)
                        console.log(`       心率: ${record.heart_rate} bpm`);
                    if (record.speed)
                        console.log(`       速度: ${record.speed.toFixed(2)} km/h`);
                    if (record.cadence)
                        console.log(`       踏频: ${record.cadence} rpm`);
                    if (record.power)
                        console.log(`       功率: ${record.power} W`);
                    if (record.temperature)
                        console.log(`       温度: ${record.temperature.toFixed(1)}°C`);
                }
            }
        }
        console.log('\n✅ FIT文件解析完成');
