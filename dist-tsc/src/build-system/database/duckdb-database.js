...
export class DuckDBDatabase {
    ...
    connection;
    dbPath;
    constructor(dbPath = 'build-system-analytics.duckdb') {
        this.dbPath = path.resolve(dbPath);
        this.db = new duckdb.Database(this.dbPath);
        this.connection = this.db.connect();
        // 将回调风格的API转换为Promise
        this.run = promisify(this.connection.run.bind(this.connection));
        // DuckDB Connection doesn't have get method, use all instead
     ...
     * 获取构建性能分析数据
     */
    async getBuildPerformanceAnalytics(startDate, endDate, buildType) {
     ...
     * 获取依赖使用分析数据
     */
    async getDependencyUsageAnalytics(packageManager, dependencyType) {
     ...
     * 获取性能趋势分析数据
     */
    async getPerformanceTrendAnalytics(metricName, environment, days) {
     ...
     * 获取构建效率分析数据
     */
    async getBuildEfficiencyAnalytics() {
        return await this.all('SELECT * FROM build_efficiency_analytics');
    }
    /**
     * 获取依赖安全分析数据
     */
    async getDependencySecurityAnalytics() {
    ...
    `);
        return {
            path: this.dbPath,
            views: views.map((row) => row.view_name)
        };
    }
}
