 ...
 */
import { ProviderStatus, ErrorCode } from '../types/index.js';
...
export class MapProviderManager extends EventEmitter {
     ...
     */
    async setProviderStatus(providerId, isActive) {
        const provider = this.providers.get(providerId);
        if (!provider) {
            throw new Error(`Provider ${providerId} not found`);
        }
     ...
     */
    async updateUsageStats(providerId) {
     ...
     */
    recordRequest(providerId, success, responseTime) {
        const stats = this.performanceStats.get(providerId);
        if (stats) {
            stats.totalRequests++;
            if (success) {
...
//# sourceMappingURL=map-provider-manager.js.map