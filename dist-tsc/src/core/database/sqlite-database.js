...
export class SQLiteDatabase {
    ...
    async getActiveProviders() {
        const rows = await this.all('SELECT * FROM map_providers WHERE is_active = 1 ORDER BY sort_order');
     ...
     */
    async getProviderStyles(providerId) {
        const rows = await this.all('SELECT * FROM map_styles WHERE provider_id = ? AND is_active = 1 ORDER BY sort_order', [providerId]);
     ...
     * Get cached tile
     */
    async getCachedTile(providerId, styleId, z, x, y) {
        const row = await this.get('SELECT * FROM tile_cache WHERE provider_id = ? AND style_id = ? AND z = ? AND x = ? AND y = ?', [providerId, styleId, z, x, y]);
     ...
     */
    async updateTileAccess(tileId) {
        await this.run('UPDATE tile_cache SET accessed_at = CURRENT_TIMESTAMP, access_count = access_count + 1 WHERE id = ?', [tileId]);
    }
    /**
     * 清理过期缓存
     * Cleanup expired cache
     */
    async cleanupExpiredCache(maxAge) {
        const expiredTime = new Date(Date.now() - maxAge);
        await this.run('DELETE FROM tile_cache WHERE accessed_at < ?', [expiredTime.toISOString()]);
    }
    /**
     * 获取缓存统计
